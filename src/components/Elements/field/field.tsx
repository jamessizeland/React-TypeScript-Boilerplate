...
const Field = forwardRef(
  (
      ...
      component = (
        <div className={style.checkboxContainer}>
          <input
            aria-required={dot}
            aria-invalid={!!error}
            className={clsx(style.checkbox, disabled ? style.disabled : '')}
            disabled={disabled}
            id={name}
            name={name}
            type="checkbox"
    ...
    return (
      <div className={clsx(style.container, disabled ? 'opacity-50' : '')}>
        <label
          htmlFor={name}
          className={clsx('text-gray-700', dot && style.dot)}
        >
          {label}
        </label>
        {component}
